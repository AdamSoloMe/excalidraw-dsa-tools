...
function detectMode(input) {
  const s = stripAssignment(input).trim();
...
async function drawLinkedList(values, x, y) {
  ...
  for (let i = 0; i < values.length; i++) {
    const beforeCell = ea.getElements().length;
    addCell(x + i * (W + GAP), y, W, H, values[i]);
    const added = ea.getElements().slice(beforeCell);
    const rect = added.find((e) => e.type === "rectangle");
    rectIds.push(rect ? rect.id : null);
  }
  await ea.addElementsToView(false, false);
  setBaseStyle();
  ea.style.endArrowHead = "arrow";
  for (let i = 0; i < values.length - 1; i++) {
    if (rectIds[i] && rectIds[i + 1]) {
      ea.connectObjects(rectIds[i], "right", rectIds[i + 1], "left", { endArrowHead: "arrow", startArrowHead: "none" });
    } else {
      ea.addArrow([[x + i * (W + GAP) + W + 10, y + H / 2], [x + (i + 1) * (W + GAP) - 10, y + H / 2]], {});
    }
  }
  groupSince(before);
  await ea.addElementsToView(false, false);
...
if (mode === "matrix") {
  ...
  await drawLinkedList(parseList(sourceText), startX, startY);
  return;
} else if (mode === "tree") {
  ...
  drawHashSet(parseList(sourceText), startX, startY);
} else if (mode === "stack") {
  drawStack(parseList(sourceText), startX, startY);
} else if (mode === "queue") {
  drawQueue(parseList(sourceText), startX, startY);
} else {
